...
def _set_ready(self, ready: bool) -> None:
    ...
    try:
        ...
        pbrowser = None  # Headless server: UI not available
    from babase._general import CallStrict
    profilename = self._profilenames[self._profileindex]

    # Handle '_edit' as a special case.
    if profilename == '_edit' and ready:
        with babase.ContextRef.empty():
            if pbrowser is not None:
    ...
    if not ready:
        self._sessionplayer.assigninput(
            ...
            CallStrict(self.handlemessage, ChangeMessage('team', -1)))
        self._sessionplayer.assigninput(
            babase.InputType.RIGHT_PRESS,
            CallStrict(self.handlemessage, ChangeMessage('team', 1)))
        self._sessionplayer.assigninput(
            babase.InputType.BOMB_PRESS,
            CallStrict(self.handlemessage, ChangeMessage('character', 1)))
        self._sessionplayer.assigninput(
            babase.InputType.UP_PRESS,
            CallStrict(self.handlemessage, ChangeMessage('profileindex', -1)))
        self._sessionplayer.assigninput(
        ...
        self._sessionplayer.assigninput(
            ...
            CallStrict(self.handlemessage, ChangeMessage('ready', 1)))
        self._ready = False
        self._update_text()
        ...
        self._sessionplayer.assigninput(
            (babase.InputType.UP_PRESS),
            CallStrict(self.handlemessage, ChangeMessage('characterchooser', -1)))
        self._sessionplayer.assigninput(
            (babase.InputType.DOWN_PRESS),
            CallStrict(self.handlemessage, ChangeMessage('characterchooser', 1)))
        self._sessionplayer.assigninput(
            (babase.InputType.BOMB_PRESS),
            CallStrict(self.handlemessage, ChangeMessage('ready', 0)))

        self._sessionplayer.assigninput(
            (babase.InputType.JUMP_PRESS, babase.InputType.PICK_UP_PRESS,
             babase.InputType.PUNCH_PRESS),
            CallStrict(self.handlemessage, ChangeMessage('ready', 2)))
