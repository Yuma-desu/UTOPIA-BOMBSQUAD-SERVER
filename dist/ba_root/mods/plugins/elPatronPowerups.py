...
try:
    ...
    browser = None  # Headless server: UI not available
...
try:
    ...
    PopupWindow = None  # Headless server: UI not available
    PopupMenu = None
...
try:
    ...
    ConfirmWindow = None  # Headless server: UI not available
...
def _bomb_init(self,
    ...
    if self.bm_type == 'ice_bubble':
        ...
        self.fire_effect_time = bs.Timer(0.1, babase.CallStrict(fire_effect, self), repeat=True)
...
def _pbx_(self, position: Sequence[float] = (0.0, 1.0, 0.0),
    ...
    if config['Powerup Time']:
        ...
        while (interval + 3):
            bs.timer(time - 1, babase.CallStrict(update_time, f'{time2}s'))
...
def new_handlemessage(self, msg: Any) -> Any:
    ...
    if isinstance(msg, bs.PickedUpMessage):
        ...
        bs.timer(0.001, babase.CallStrict(self._hit_self, msg.intensity))
        ...
        if msg.poweruptype == 'triple_bombs':
            ...
            if self.powerups_expire:
                ...
                self.ice_man_flash_timer = (bs.Timer(
                    ice_man_time - 2000,
                    babase.CallStrict(_ice_man_off_flash, self)))

                self.ice_man_timer = (bs.Timer(ice_man_time,
                                               babase.CallStrict(_ice_man_wear_off,
                                                           self)))
            ...
            if self.powerups_expire:
                ...
                self.speed_flash_timer = (bs.Timer(
                    speed_time - 2000,
                    babase.CallStrict(_speed_off_flash, self)))

                self.speed_timer = (bs.apptimer(speed_time,
                                                babase.CallStrict(_speed_wear_off,
                                                            self)))
        ...
        if not self.frozen:
            ...
            bs.timer(5.0, babase.CallStrict(self.handlemessage,
                                      bs.ThawMessage()))
            if self.hitpoints <= 0:
                self.shatter()
        ...
        if msg.hit_subtype == 'fly':
            ...
            if not self.shield:
                for firex in range(duration):
                    bs.timer(index, babase.CallStrict(fire, index, damage))
                    self._fire_time = bs.Timer(0.1, babase.CallStrict(fire_effect),
                                               repeat=True)
                    index += 1
            else:
        ...
        if self.hitpoints > 0:
            ...
            if self._cursed and damage > 0:
                bs.timer(
                    0.05,
                    babase.CallStrict(self.curse_explode,
...
class PowerupManagerWindow(PopupWindow):
        ...
        for tab in range(self.count):
            for tab2 in range(columns):
                ...
                self.tab_buttons[tag] = bui.buttonwidget(
                    ...
                    button_type='square',
                    on_activate_call=babase.CallStrict(self._set_tab, tag,
                                                 sound=True))
    ...
    def _buy_object(self, tag: str, price: int):
        store = BearStore(value=tag, price=price,
                          callback=babase.CallStrict(self.store_refresh, tag))
        store.buy()
