    ...
    with babase.Context(player.actor._activity()):
        ...
        for move in stunt:
            ...
            if value is None:
                bs.timer(
                    move["time"],
                    babase.CallStrict(player.actor.move_map[move["move"]["action"]])
                )
            else:
                bs.timer(
                    move["time"],
        ...
        bs.timer(time_to_hide_controls,
                 babase.CallStrict(handle_player_replay_end, player))
    ...
    with player.actor._activity().context:
        ...
        for move in stunt:
            ...
            if value is None:
                bs.timer(
                    move["time"],
                    babase.CallStrict(bot.move_map[move["move"]["action"]])
                )
                ui_activation = CONTROLS_UI_MAP.get(move["move"]["action"])
                if ui_activation:
                    bs.timer(
                        move["time"],
                        babase.CallStrict(ui_activation, player.actor._activity())
                    )
            else:
                bs.timer(
                    move["time"],
        ...
        bs.timer(time_to_hide_controls,
                 babase.CallStrict(handle_player_replay_end, player))
        bs.timer(time_to_hide_controls, babase.CallStrict(bot.node.delete))
