...
class Player(bs.Player['Team']):
    ...
    def set_state(self, state: PlayerState) -> None:
        ...
        if old_state != PlayerState.STUNNED and state == PlayerState.STUNNED:
            ...
            self.stunned_timer = bs.Timer(stun_time + 0.1, babase.CallStrict(self.stun_remove))
            self.stunned_update_timer = bs.Timer(0.1, babase.CallStrict(
...
class HotPotato(bs.TeamGameActivity[Player, bs.Team]):
    ...
    def marked_players_died(self) -> bool:
        ...
        if len(alive_players) < 2:
            ...
            self._end_game_timer = bs.Timer(1.25, babase.CallStrict(self.end_game))
        else:
            # There's still players remaining, so let's wait a while before marking a new player.
            self.new_mark_timer = bs.Timer(
                2.0 if self.slow_motion else 4.0, babase.CallStrict(self.new_mark))
    ...
    def new_mark(self) -> None:
        ...
        self.marked_tick_timer = bs.Timer(1.0, babase.CallStrict(self._eliminate_tick), repeat=True)
    ...
    def on_begin(self) -> None:
        ...
        if len(self.players) < 2:
            ...
            self.new_mark_timer = bs.Timer(
                2.0 if self.slow_motion else 5.2, babase.CallStrict(self.new_mark))
