...
async def on_message(message):
    ...
    if message.channel.id == logsChannelID:
        _babase.pushcall(CallStrict(bs.chatmessage,
...
class BsDataThread(object):
    def __init__(self):
        self.refreshStats()
        self.Timer = bs.AppTimer(8, babase.CallStrict(self.refreshStats), repeat=True)
        # self.Timerr = bs.Timer( 10,babase.CallStrict(self.refreshLeaderboard),timetype = babase.TimeType.REAL,repeat = True)
